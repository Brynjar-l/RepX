-- =====================================================
-- RepX: FULL CLEAN RESET + SCHEMA CREATE
-- =====================================================

-- ---------- DROP VIEWS (if any) ----------
DROP VIEW IF EXISTS v_set_volume CASCADE;
DROP VIEW IF EXISTS v_recent_workouts CASCADE;

-- ---------- DROP ALL TABLES (order-safe) ----------
DO $$
DECLARE r RECORD;
BEGIN
  FOR r IN (
...
CREATE INDEX idx_wex_workout  ON workout_exercises(workout_id, order_index);
...
CREATE INDEX idx_sets_wex ON sets(workout_exercise_id, set_index);
...
CREATE INDEX idx_tex_template ON template_exercises(template_id, order_index);
...
CREATE INDEX idx_photos_user_time ON progress_photos(user_id, taken_at DESC);
